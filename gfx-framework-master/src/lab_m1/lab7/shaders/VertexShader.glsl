...
void main()
{
    ...
    float diffuse_light = 0.8 * material_kd * max(dot(world_normal,L),0);

    // TODO(student): Compute specular light component

    float f = 1 / (distance(world_pos, light_position) * distance(world_pos, light_position)  + distance(world_pos, light_position) + 1 );
    ...
    if (diffuse_light > 0)
    {
        specular_light = 10 * material_ks * pow(max(dot(R,V), 0), material_shininess);
    } 
    
    // TODO(student): Compute light
    float light = ambient_light + f* diffuse_light + f * specular_light;
