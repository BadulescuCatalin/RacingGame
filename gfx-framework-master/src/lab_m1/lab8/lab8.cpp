...
{
    {
        ...
        // TODO(student): Add or change the object colorsy
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.75, 0.15, 0.25));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0.75, 0.3));
    ...
    // second sphere( point light)
    {
        glm::mat4 modelMatrix = glm::mat4(1) + glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix , lightPosition + glm::vec3(1,1,1));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_M)) angle += RADIANS(5) * deltaTime * 30;
        if (window->KeyHold(GLFW_KEY_N)) angle -= RADIANS(5) * deltaTime * 30;

        // TODO(student): Set any other keys that you might need
        if (window->KeyHold(GLFW_KEY_O)) {
            angle += RADIANS(20) * deltaTime;
        }
